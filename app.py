...
if fichier_contacts and fichier_factures and fichier_mapping and fichier_simu:
    if st.button("🚀 Lancer le calcul", type="primary", use_container_width=True):
        ...
        with st.spinner("Analyse et fusion des bases de données en cours..."):
            try:
                # ---------------------------------------------------------
                # MAPPING BIDIRECTIONNEL (Commun aux deux)
                # ---------------------------------------------------------
                ...
                # =================================================================================================
                    ...
                    m_simules = set(df_sim_final['Proprietaire']); m_factures = set(df_reels_final['Proprietaire'])
                    simu_jamais_fact = list(m_simules - m_factures); fact_jamais_sim = list(m_factures - m_simules)
                    ...
                    if not inconnus.any() and not simu_sans_map and not fact_jamais_sim and not simu_jamais_fact:
                    ...
                    col_a1.metric("⚡ Total Consommé 12 mois", f"{t_rc:.2f} MWh", f"{pc_c:+.1f}% (Annuel)", delta_color="off")
                    col_a2.metric("☀️ Total Produit 12 mois", f"{t_rp:.2f} MWh", f"{pc_p:+.1f}% (Annuel)", delta_color="off")
                    col_a3.metric("🤝 Total Échangé 12 mois", f"{t_re:.2f} MWh", f"{pc_e:+.1f}% (Annuel)", delta_color="off")
                    st.divider()
                    ...
                    st.markdown("*Gris = Pas de facture Sibelga ce mois-là.*")
                    ...
                    st.subheader("👤 Profil Individuel Annuel")
                ...
                nom_fichier = f"Audit_Mois_{mois_cible}.csv" if mode_analyse == "📅 Mensuel (Contrôle de facture)" else "Audit_Annuel_Global.csv"
