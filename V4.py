...
def parser_simu_15min(fichier_bytes):
    ...
    for col in df_s.columns:
        if '_residual' in col: membres_simu.add(col.split('_residual')[0].strip())
        elif '_shared' in col: membres_simu.add(col.split('_shared')[0].strip())
            ...
            try:
                ...
                if fichier_contacts.name.lower().endswith('.csv'):
                    try:
                        ...
                        if len(df_contacts.columns) < 2:
                            fichier_contacts.seek(0)
                            df_contacts = pd.read_csv(fichier_contacts, sep=',', dtype=str)
                    except:
                        fichier_contacts.seek(0)
                        df_contacts = pd.read_csv(fichier_contacts, sep=',', dtype=str)
                ...
                if mode_precision == "Analyse Mensuelle (Rapide)":
                    ...
                    for fact in fichiers_sibelga:
                        fact.seek(0)
                        if fact.name.lower().endswith('.csv'):
                            try:
                                df_r = pd.read_csv(fact, sep=';', dtype=str)
                                if len(df_r.columns) < 3:
                                    fact.seek(0)
                                    df_r = pd.read_csv(fact, sep=',', dtype=str)
                            except:
                        ...
                        for c in [c_vol_part, c_vol_comp, c_inj_part, c_inj_comp]:
                            df_r[c] = pd.to_numeric(df_r[c].astype(str).str.replace(',', '.'), errors='coerce', downcast='float').fillna(0)
                    ...
                    for m_encours, group in df_s.groupby('Mois_Simu'):
                        ...
                        for p in p_simu:
                            d_simu.append({
                                ...
                                'Sim_Conso_Partagee_MWh': s_simu.get(f"{p}_shared volume from community", 0) / 1000.0,
                                'Sim_Conso_Totale_MWh': (s_simu.get(f"{p}_residual off-take", 0) + s_simu.get(f"{p}_shared volume from community", 0)) / 1000.0,
                                'Sim_Prod_Partagee_MWh': abs(s_simu.get(f"{p}_shared volume to community", 0)) / 1000.0,
                                'Sim_Prod_Totale_MWh': (abs(s_simu.get(f"{p}_residual injection", 0)) + abs(s_simu.get(f"{p}_shared volume to community", 0))) / 1000.0,
                ...
                # ==========================================
                    ...
                    cols_simu = ['Proprietaire', 'Join_Key'] + cols_to_sum
...
if st.session_state.get('calcul_termine', False):
    ...
    sim_conso_res = df_mensuel_agg['Sim_Conso_Totale_MWh'] - df_mensuel_agg['Sim_Conso_Partagee_MWh']
    sim_prod_res = df_mensuel_agg['Sim_Prod_Totale_MWh'] - df_mensuel_agg['Sim_Prod_Partagee_MWh']

    p_achat_res = safe_div(df_mensuel_agg['Sim_Cout_Reseau_Euro'], sim_conso_res)
    ...
    if vue_choisie == "📅 Vue Mensuelle (Détail)":
        ...
        tot_r_ben = df_mensuel['Reel_Benefice'].sum()
        tot_s_ben = df_mensuel['Sim_Benefice'].sum()
        ...
        col_m4.metric("💰 Bénéfice Financier", f"{tot_r_ben:.2f} €", f"Simu: {tot_s_ben:.2f} €", delta_color="off")
    ...
    # =========================================================
        ...
        tot_r_ben_cum = df_apples['Reel_Benefice'].sum()
        tot_s_ben_cum = df_apples['Sim_Benefice'].sum()
        ...
        col_a4.metric("💰 Bénéfice Financier (Cumulé)", f"{tot_r_ben_cum:.2f} €", f"Simu: {tot_s_ben_cum:.2f} €", delta_color="off")
        st.divider()
        ...
        # ===============================================
        # ANALYSE INDIVIDUELLE (AVEC OPTION FINANCE)
        # ===============================================
        ...
        if not df_indiv.empty:
            ...
            if choix_kpi_indiv != "💰 Finances (Gains & Dépenses)":
                ...
                df_indiv_plot = df_indiv.groupby('Axe_Temps')[[
                    ...
                    'Reel_Prod_Partagee_MWh', 'Sim_Prod_Partagee_MWh'
                ]].sum().reset_index()
            ...
            # ============================
            # SI MODE FINANCES (RECAP)
            # ============================
            else:
                df_finance = df_mensuel_agg[(df_mensuel_agg['Proprietaire'] == membre_choisi) & (df_mensuel_agg['Has_Facture'] > 0)]
                
                r_cout_sans = df_finance['Reel_Cout_Sans_CE'].sum()
                r_cout_avec = df_finance['Reel_Cout_Avec_CE'].sum()
                r_rev_sans = df_finance['Reel_Revenu_Sans_CE'].sum()
                r_rev_avec = df_finance['Reel_Revenu_Avec_CE'].sum()
                r_ben = df_finance['Reel_Benefice'].sum()

                s_cout_sans = df_finance['Sim_Cout_Sans_CE'].sum()
                s_cout_avec = df_finance['Sim_Cout_Avec_CE'].sum()
                s_rev_sans = df_finance['Sim_Revenu_Sans_CE'].sum()
                s_rev_avec = df_finance['Sim_Revenu_Avec_CE'].sum()
                s_ben = df_finance['Sim_Benefice'].sum()

                st.markdown(f"#### Bilan Financier de {membre_choisi}")
                col_f1, col_f2, col_f3 = st.columns(3)
                
                col_f1.metric("Dépenses Totales (Avec CE)", f"{r_cout_avec:.2f} €", f"Simu: {s_cout_avec:.2f} €", delta_color="inverse")
                col_f2.metric("Revenus Totaux (Avec CE)", f"{r_rev_avec:.2f} €", f"Simu: {s_rev_avec:.2f} €", delta_color="normal")
                col_f3.metric("Bénéfice Réel Généré par la CE", f"{r_ben:.2f} €", f"Simu: {s_ben:.2f} €", delta_color="normal")

                st.markdown("##### Détail Comparatif")
                finance_data = pd.DataFrame({
                    "Scénario Économique": ["Situation SANS Communauté d'Énergie", "Situation AVEC Communauté d'Énergie", "Bénéfice (Gain ou Économie générée)"],
                    "Dépenses Réelles": [f"{r_cout_sans:.2f} €", f"{r_cout_avec:.2f} €", f"{r_cout_sans - r_cout_avec:.2f} €"],
                    "Dépenses Simulées": [f"{s_cout_sans:.2f} €", f"{s_cout_avec:.2f} €", f"{s_cout_sans - s_cout_avec:.2f} €"],
                    "Revenus Réels": [f"{r_rev_sans:.2f} €", f"{r_rev_avec:.2f} €", f"{r_rev_avec - r_rev_sans:.2f} €"],
                    "Revenus Simulés": [f"{s_rev_sans:.2f} €", f"{s_rev_avec:.2f} €", f"{s_rev_avec - s_rev_sans:.2f} €"]
                })
